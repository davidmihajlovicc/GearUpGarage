...
export default function Payment(){
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try{
        ...
        if (!c || !c.items?.length) { nav('/cart'); return; }
        setCart(c); setProfile(p);
  ...
  async function payNow(){
    setErr('');
    try{
      await checkout();                 // finalizira + prazni košaricu (backend provjerava profil)
      alert('Payment successful. Thank you!');
      nav('/parts');
    }catch(e){
      const msg = e?.response?.data?.error || 'Error occured with payment.';
      setErr(msg);
      if (e?.response?.status === 400) nav('/checkout'); // profil nije potpun
    }
  }
      ...
      <div className="page payment-page">
        ...
        <div className="payment-grid">
          ...
          <div className="payment-form">
            ...
            {/* mock polja – ovdje ide integracija (Stripe/CorvusPay…) */}
            ...
            <div className="payment-actions">
              <Link className="btn btn--ghost" to="/checkout">Back</Link>
          ...
          <aside className="payment-summary">
            ...
            {profile ? (
              <div className="muted delivery-info">
                ...
                {profile.address_line1} {profile.house_no}{profile.address_line2 ? `, ${profile.address_line2}`:''}<br/>
                {profile.postal_code} {profile.city}, {profile.country}<br/>
                Tel: {profile.phone}
              </div>
            ) : (
              <div className="muted">No data. <Link to="/checkout">Enter data</Link></div>
            )}
          </aside>
        </div>
      </div>
